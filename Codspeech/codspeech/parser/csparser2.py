# ----------------------------------------------------------------------
# Codspeech/codspeech/parser: csparser2.py
#
# A parser for Codspeech
# ----------------------------------------------------------------------

from ..ply      import yacc
from ..ast      import csast2
...
class CodspeechParser(PLYParser):
    ...
    def _build_component(name, doc, in_params, out_params, body):
        return csast2.Component(
            name=name,
            doc=doc,
            in_params=in_params,
    ...
    def p_program(self, p):
        ...
        """
        if len(p) == 5:
            p[0] = csast2.Program(p[1], p[3])
        elif len(p) == 4:
            p[0] = csast2.Program(p[1], [])
        elif len(p) == 3:
            p[0] = csast2.Program([], p[1])
        else:
            p[0] = csast2.Program([], [])
    ...
    def p_component_decl(self, p):
        ...
        """
        hdr = p[2]
        p[0] = csast2.Component(
            name=hdr[0],
            doc=csast2.Docstring(hdr[1]),
            in_params=hdr[2],
            out_params=hdr[3],
            body=p[3])
    ...
    def p_in_param(self, p):
        ...
        if len(p) == 4:
            p[0] = csast2.InParam(
                doc=csast2.Docstring(p[3]),
                type=csast2.Type(p[1]),
                name=p[2],
                opt=csast2.Optional(False),
                default=None)
        elif len(p) == 5:
            p[0] = csast2.InParam(
                doc=csast2.Docstring(p[4]),
                type=csast2.Type(p[2]),
                name=p[3],
                opt=csast2.Optional(True),
                default=None)
        else:
            p[0] = csast2.InParam(
                doc=csast2.Docstring(p[5]),
                type=csast2.Type(p[1]),
                name=p[2],
                opt=csast2.Optional(False),
                default=p[4])
    ...
    def p_network_stmt(self, p):
        ...
        """
        if len(p) == 3:
            p[0] = csast2.Network(None, p[2])
        else:
            p[0] = csast2.Network(p[3], p[4])


    # A network controller is a component and the controller alias?
    def p_network_controller(self, p):
        """
        network_controller : CONTROLLER ident
        """
        p[0] = csast2.Controller(p[2])
    ...
    def p_atom_stmt(self, p):
        ...
        """
        p[0] = csast2.Atom(csast2.AtomType(p[2]),p[5])
    ...
    def p_atom_conf(self, p):
        ...
        """
        p[0] = csast2.AtomConf(p[1].value, p[3].value)
    ...
    def p_stmt_block(self, p):
        ...
        """
        if len(p) == 5:
            p[0] = csast2.StmtBlock(p[3])
        else:
            p[0] = csast2.StmtBlock(p[2])
    ...
    def p_fconst(self, p):
        ...
        fconst : FCONST
        """
        p[0] = csast2.Constant('Float', p[1], self._coord(p.lineno(1)))
    ...
    def p_iconst(self, p):
        ...
        iconst : ICONST
        """
        p[0] = csast2.Constant('Int', p[1], self._coord(p.lineno(1)))
    ...
    def p_sconst(self, p):
        ...
        sconst : SCONST
        """
        p[0] = csast2.Constant('String', p[1], self._coord(p.lineno(1)))
    ...
    def p_ident(self, p):
        ...
        ident : ID
        """
        p[0] = csast2.ID(p[1],coord=self._coord(p.lineno(1)))
    ...
    def p_param_ref(self, p):
        ...
        """
        if len(p) == 4:
            p[0] = csast2.ParamRef(None,p[1],p[3])
        elif len(p) == 6:
            p[0] = csast2.ParamRef(p[1],p[3],p[5])
        else:
            p[0] = csast2.ParamRef(p[2],p[5],p[7])
