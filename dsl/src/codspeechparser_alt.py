...
class CodspeechParser(PLYParser):
    ...
    def p_in_param(self, p):
        ...
        if len(p) == 4:
            p[0] = csast.InParam(
                doc=csast.Docstring(p[3]),
                type=csast.Type(p[1]),
                name=p[2],
                opt=csast.Optional(False),
                default=None)
        elif len(p) == 5:
            p[0] = csast.InParam(
                doc=csast.Docstring(p[4]),
                type=csast.Type(p[2]),
                name=p[3],
                opt=csast.Optional(True),
                default=None)
        else:
            p[0] = csast.InParam(
                doc=csast.Docstring(p[5]),
                type=csast.Type(p[1]),
                name=p[2],
                opt=csast.Optional(False),
                default=p[4])
    ...
    def p_network_stmt(self, p):
        ...
        """
        if len(p) == 3:
            p[0] = csast.Network(None, p[2])
        else:
            p[0] = csast.Network(p[3], p[4])
    ...
    def p_atom_conf(self, p):
        ...
        """
        p[0] = csast.AtomConf(p[1].value, p[3].value)
    ...
    def p_stmt_block(self, p):
        ...
        """
        if len(p) == 5:
            p[0] = csast.StmtBlock(p[3])
        else:
            p[0] = csast.StmtBlock(p[2])
    ...
    def p_fconst(self, p):
        ...
        fconst : FCONST
        """
        p[0] = csast.Constant('Float', p[1], self._coord(p.lineno(1)))
    ...
    def p_iconst(self, p):
        ...
        iconst : ICONST
        """
        p[0] = csast.Constant('Int', p[1], self._coord(p.lineno(1)))
    ...
    def p_sconst(self, p):
        ...
        sconst : SCONST
        """
        p[0] = csast.Constant('String', p[1], self._coord(p.lineno(1)))
    ...
    def p_ident(self, p):
        ...
        ident : ID
        """
        p[0] = csast.ID(p[1],coord=self._coord(p.lineno(1)))
    ...
    def p_param_ref(self, p):
        ...
        """
        if len(p) == 4:
            p[0] = csast.ParamRef(None,p[1],p[3])
        elif len(p) == 6:
            p[0] = csast.ParamRef(p[1],p[3],p[5])
        else:
            p[0] = csast.ParamRef(p[2],p[5],p[7])
